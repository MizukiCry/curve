...
inline std::string CalcRelativePath(const std::string &path1,
    ...
    while (unmatchedIndex < static_cast<int>(dirs1.size()) &&
           unmatchedIndex < static_cast<int>(dirs2.size())) {
    ...
    std::string rpath;
    if (unmatchedIndex == static_cast<int>(dirs1.size())) {
        rpath.append(".");
    }
    ...
    for (int i = unmatchedIndex; i < static_cast<int>(dirs2.size()); ++i) {
        rpath.append("/");
